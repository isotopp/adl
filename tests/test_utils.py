from context import utils
from unittest.mock import MagicMock
import unittest
import base64
import os


class TestUtils(unittest.TestCase):
    def test_aes_crypt(self):
        ur = os.urandom
        os.urandom = MagicMock(
            return_value=base64.b64decode("2FfII6r4sz2Mgl9QyspUlQ==")
        )
        cleartext = ""
        devicekey = "7Nc0TFtBePQ2VDSzPJMmWg=="

        crypted = utils.aes_crypt(
            base64.b64decode(cleartext), base64.b64decode(devicekey)
        )
        self.assertEqual(
            crypted,
            b"",
        )

        plaintext = utils.aes_decrypt(
            base64.b64decode(crypted), base64.b64decode(devicekey)
        )
        self.assertEqual(
            plaintext,
            b"",
        )

        os.urandom = ur

    def test_generate_key(self):
        prvk, pubk = utils.generate_key_pair()


if __name__ == "__main__":
    unittest.main()
