...
class DB:
    ...
    def load_config(self):
        ...
        # Config
        rows = c.execute(
            "select default_user, auth_url, activation_certificate, userinfo_url, authentication_certificate from configuration"
        )
    ...
    def load_accounts(self):
        ...
        accounts = []
        rows = c.execute(
            "select user_id, sign_id, sign_method, auth_pub, auth_priv, license_pub, license_priv, pkcs12, eplk, license_certificate from users"
        )
        for user in rows.fetchall():
            ...
            # Devices
            rows = c.execute(
                "select device_name, device_key, device_id, fingerprint, device_type from devices where user_id=?",
                (a.urn,),
            )
    ...
    def store_config(self, conf):
        ...
        c.execute(
            "insert into configuration (default_user, auth_url, activation_certificate, userinfo_url, authentication_certificate) values (?,?,?,?,?)",
            values,
        )
    ...
    def add_account(self, a):
        ...
        c.execute(
            "insert into users(user_id, sign_id, sign_method, auth_pub, auth_priv, license_pub, license_priv, pkcs12, eplk, license_certificate)  values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ph,
        )
    ...
    def add_device(self, account_urn, d):
        ...
        c.execute(
            "insert into devices(user_id, device_key, device_id, fingerprint, device_name, device_type) values(?, ?, ?, ?, ?, ?)",
            dph,
        )
