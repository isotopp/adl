...
def device_register(mountpoint):
    ...
    if username is not None and plk is not None:
        ...
        if a is not None:
            if a.get_private_key() == plk:
                ...
                logging.info(
                    "Device is already activated for user {} but is not known by adl, registering it".format(
                        username
                    )
                )
        ...
        logging.error(
            "Device is already activated for an unknown user. Doing nothing or all books on the device will become unreadable"
        )
        return
    ...
    activation_file_content = build_activation_file(
        current_account, activation_token, data.config
    )
...
def read_device_file(mountpoint):
    try:
        with open(
            "{}/.adobe-digital-editions/device.xml".format(mountpoint), "r"
        ) as device_file:
            d = Device()
            ...
            d.fingerprint = tree_root.find(
                "{http://ns.adobe.com/adept}fingerprint"
            ).text
...
def read_activation_file(mountpoint):
    try:
        with open(
            "{}/.adobe-digital-editions/activation.xml".format(mountpoint), "r"
        ) as activation_file:
            ...
            logging.info(
                "This device is already activated for user {}".format(username)
            )
...
def write_activation_file(mountpoint, content):
    try:
        with open(
            "{}/.adobe-digital-editions/activation.xml".format(mountpoint), "w"
        ) as activation_file:
