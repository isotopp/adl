...
class FFAuth(APICall):
    ...
    def build(self):
        ...
        add_subelement(
            ff, "authenticationCertificate", self.config.authentication_certificate
        )
...
class InitLicense(APICall):
    ...
    def build(self):
        ff = etree.Element(
            "{%s}licenseServiceRequest" % ADEPT_NS,
            nsmap=NSMAP,
            attrib={"identity": "user"},
        )
...
class Activate(APICall):
    ...
    def build(self):
        xml = etree.Element(
            "{%s}activate" % ADEPT_NS, nsmap=NSMAP, attrib={"requestType": "initial"}
        )
        ...
        add_subelement(
            xml, "clientLocale", "en"
        )  # TODO: is it useful to set the real locale ?
...
class ActivationInit(APICall):
    ...
    def parse(self, reply):
        ...
        activation_certificate = tree_root.find(
            "{http://ns.adobe.com/adept}certificate"
        ).text
...
class SignInDirect(APICall):
    ...
    def build(self):
        xml = etree.Element(
            "{%s}signIn" % ADEPT_NS, nsmap=NSMAP, attrib={"method": self.sign_method}
        )
    ...
    def parse(self, reply):
        ...
        epk = tree_root.find(
            "{http://ns.adobe.com/adept}encryptedPrivateLicenseKey"
        ).text
