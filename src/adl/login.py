...
def login(user, password):
    ...
    if not config.ready():
        config.auth_url, config.userinfo_url, config.activation_certificate = (
            activation_init()
        )
...
def sign_in(data, acc, user, password):
    ...
    if user is None or password is None:
        ...
        logging.warning(
            "WARNING: logging in as anonymous - please see README for risks"
        )
    else:
    ...
    certificate = x509.load_der_x509_certificate(
        base64.b64decode(data.config.authentication_certificate)
    )
    ...
    encrypted_auth_data = public_auth_key.encrypt(
        serialized_auth_data, asym_padding.PKCS1v15()
    )

    # Encrypt key pairs with device key
    encrypted_akp = (
        utils.aes_crypt(base64.b64decode(acc.auth_key[0]), binary_device_key),
        acc.auth_key[1],
    )
    encrypted_lkp = (
        utils.aes_crypt(base64.b64decode(acc.license_key[0]), binary_device_key),
        acc.license_key[1],
    )

    signin = SignInDirect(
        acc.sign_method, encrypted_auth_data, encrypted_akp, encrypted_lkp
    )
    success, acc.urn, acc.pkcs12, acc.encryptedPK, acc.licenseCertificate = (
        signin.call()
    )
